														...
														.ajax({
															...
															success : function(
																...
																for (var i = 0; i < noSubs.length; i++) {
																	noSubsStr += "<li data-subid="+subs[i].id+">"
																			+ noSubs[i].name
																			+ "</li>"
																}
																$("#noSub")
...
<body>
	...
	<div id="main">
		...
		<ul id="sub">
			<c:forEach var="sub" items="${bj.subs }">
				<li data-subid="${sub.id }">${sub.name }</li>
			</c:forEach>
		</ul>
		<div class="btnGroup">
			<button type="button" id="add" class="btn btn-danger">↑</button>
			<button type="button" id="delete" class="btn btn-danger">↓</button>
		</div>
		<div id="mes"></div>
		<ul id="noSub">
